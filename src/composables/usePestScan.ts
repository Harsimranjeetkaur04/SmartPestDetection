...
export function usePestScan() {
  ...
  const uploadPestScan = async (file: File): Promise<string | void> => {
    ...
    try {
      ...
      const imageUrl = `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/store/${fileName}`;
      ...
      const storedImageUrl = `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/store/${processedFileName}`;
